...
struct IKCPSEG
{
	struct IQUEUEHEAD node; // 队列指针
	IUINT32 conv;   // 区分kcp会话
	IUINT32 cmd;    // 数据包类型
	IUINT32 frg;    // 分段后的分段数
	IUINT32 wnd;    // 窗口大小
	IUINT32 ts;     // 时间戳
	IUINT32 sn;     // 分配的sn序列号
	IUINT32 una;    // 累计确认的数据包编号
	IUINT32 len;    // 长度
	IUINT32 resendts;   // 重传时间戳，超过这个时间重发这个包
	IUINT32 rto;        // 超时重传数据
	IUINT32 fastack;    // ack略过的次数, 主要用于快传
...
struct IKCPCB
{
    ...
    // nrcv_buf 接收buf长度，nsnd_buf 发送大小长度
	IUINT32 nrcv_buf, nsnd_buf;
    // nrcv_que 接收队列长度，nsnd_que 发送队列大小长度
